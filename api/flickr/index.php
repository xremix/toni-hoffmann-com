...
foreach ($galleryPhotos as $galleryPhoto) {
	...
	// https://www.flickr.com/services/api/misc.urls.html
	//500px
	$obj["thumbnail"] = $galleryPhoto['farm'].'-'.$galleryPhoto['server'].'-'.$galleryPhoto['id'].'_'.$galleryPhoto['secret'].'.jpg';
	//640px
	$obj["middleurl"] = $galleryPhoto['farm'].'-'.$galleryPhoto['server'].'-'.$galleryPhoto['id'].'_'.$galleryPhoto['secret'].'_z.jpg';
	//1024px
	$obj["url"] = $galleryPhoto['farm'].'-'.$galleryPhoto['server'].'-'.$galleryPhoto['id'].'_'.$galleryPhoto['secret'].'_b.jpg';
	//Original
	$obj["bigurl"] = $galleryPhoto['farm'].'-'.$galleryPhoto['server'].'-'.$galleryPhoto['id'].'_'.$privateSecret.'_o.'.$galleryPhoto["originalformat"];
	$obj["originalurl"] = $galleryPhoto['farm'].'-'.$galleryPhoto['server'].'-'.$galleryPhoto['id'].'_'.$privateSecret.'_o.'.$galleryPhoto["originalformat"];
	$obj["linkUrl"] = 'https://www.flickr.com/photos/'.$result['photoset']['owner'].'-'.$galleryPhoto['id'];
	//Cut out everyting in [brackets]
	$obj["title"] = preg_replace('/[\[{\(].*[\]}\)]/U' , '', $galleryPhoto['title']);
	$obj["alt"] = preg_replace('/[\[{\(].*[\]}\)]/U' , '', $galleryPhoto['title']).' from Toni Hoffmann';
